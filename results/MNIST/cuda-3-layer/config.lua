...
--layer2
encoder:add(nn.SpatialZeroPaddingCUDA(pad2,pad1,pad2,pad1))
encoder:add(nn.SpatialConvolutionCUDA(feature_maps,feature_maps_2,filter_size_2,filter_size_2,stride,stride))
encoder:add(nn.Threshold(0,1e-6))

--layer3
encoder:add(nn.SpatialZeroPaddingCUDA(pad_2,pad_2,pad_2,pad_2)) 
encoder:add(nn.SpatialConvolutionCUDA(feature_maps_2,feature_maps_3,filter_size_3,filter_size_3,1,1))
encoder:add(nn.Threshold(0,1e-6))
...
decoder:add(nn.SpatialConvolutionCUDA(feature_maps_3,feature_maps_2,filter_size_2,filter_size_2,1,1))
decoder:add(nn.Transpose({4,1},{4,2},{4,3}))
--layer2
decoder:add(nn.Reshape((map_size_2^2)*batchSize,feature_maps))
decoder:add(nn.SpatialDeconvolution(feature_maps_2,feature_maps,factor))
decoder:add(nn.Threshold(0,1e-6))
