local SpatialZeroPaddingC, parent = torch.class('nn.SpatialZeroPaddingC', 'nn.Module')

function SpatialZeroPaddingC:__init(pad_l, pad_r, pad_t, pad_b)
   parent.__init(self)
   self.pad_l = pad_l
...
function SpatialZeroPaddingC:updateOutput(input)
   if input:dim() == 3 then
      -- sizes
...
function SpatialZeroPaddingC:updateGradInput(input, gradOutput)
   if input:dim() == 3 then 
