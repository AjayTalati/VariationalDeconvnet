...
Model $P(\mathbf{X},\mathbf{Z}) = \frac{P(\mathbf{Z})\cdot P(\mathbf{X}|\mathbf{Z})}{P(\mathbf{Z}|\mathbf{X})}$ to find structure in data $\mathbf{X}$. \\ 
Here, $P(\mathbf{X}|\mathbf{Z})$ and $P(\mathbf{Z}|\mathbf{X})$ are modeled by the Convolutional layers and the Deconvolutional layers, respectively.
\end{frame}
